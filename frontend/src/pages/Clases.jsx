    ...
    <div className="p-6">
      ...
      {auth?.rol === "docente" && (
        <button
          ...
          className="text-white px-4 py-2 rounded transition hover:bg-[#147BB2]"
          style={{ backgroundColor: '#179FCE' }}
        >
          Crear Clase
        </button>
      )}
      ...
      <div className="overflow-x-auto mt-6 shadow-lg  border border-black">
        <table className="min-w-full bg-white  overflow-hidden text-lg">
          <thead>
          ...
          <tbody className="divide-y divide-black">
            {clases.length === 0 ? (
              ...
              clases.map((clase, index) => (
                  <tr
                    key={clase.id}
                    className={`${index % 2 === 0 ? "bg-gray-50" : "bg-f9f8fe"} border-b border-white`}
                    
                  >
                  <td className="p-4 text-gray-700 text-center border-r border-black">{clase.materia?.nombre || "Sin nombre"}</td>
                  <td className="p-4 text-gray-700 text-center border-r border-black">{clase.curso?.nombre || "Sin curso"}</td>
                  <td className="p-4 text-gray-700 text-center border-r border-black">
                    
                    {format(new Date(clase.fecha), "dd/MM/yyyy")}
                  </td>
                  <td className="p-4 text-gray-700 text-center border-r border-black">{clase.hora_inicio}</td>
                  <td className="p-4 text-gray-700 text-center border-r border-black">{clase.hora_fin}</td>
                  <td className="p-4 text-center flex justify-center">
                    {auth?.rol === "docente" && clase.docente_id === auth.docente_id && (
                      <button
                        onClick={() => generarQR(clase)}
                        className="bg-[#375982] text-white px-3 py-1 rounded text-lg hover:bg-[#2c4668] transition"
                      >
                        Generar QR
                      </button>
                    )}
      ...
      {mostrarModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <FormularioClase onClose={() => setMostrarModal(false)} />
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
          <div className="bg-white p-6 rounded-lg max-w-md text-center">
            <h2 className="text-xl font-semibold mb-4">
              Código QR para: {claseQR.materia?.nombre}
            </h2>
